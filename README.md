...
- **PID** read — SmartPlant / Smart P&ID sheets, via [`pid-parse`](../pid-parse). Imports the source-backed Sheet geometry (lines, polylines, points, text, symbol placements) onto `PID-*` layers, drawing each symbol body from the external `.sym` library, at the width, colour and dashed linetype the drawing's own style table states; where a published `_Data.xml` sits beside the sheet, entities carry their pipeline / equipment identity.
