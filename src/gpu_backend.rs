...
fn probe_software_adapters() -> Option<ProbeCaps> {
    ...
    if software.is_empty() {
        ...
        let adapter = pollster::block_on(instance.request_adapter(&wgpu::RequestAdapterOptions {
            ...
            // wgpu 30 (the local iced fork): bucketed limits are a
            // fingerprinting defence for untrusted content, not for a probe.
...
fn staging_upload(device: &iced::wgpu::Device) -> bool {
    ...
    {
        // wgpu 30 hands the mapping back as a Result; a range the driver
        // refuses is the probe failing, not a panic.
        let Ok(mut view) = slice.get_mapped_range_mut() else {
            return false;
        };
...
"#;
    ...
    if slice.get_mapped_range().is_err() {
        return false;
    }
    true
}
