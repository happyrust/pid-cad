...
/// The plot dialog's SVG destination, stored as itself: no other application
/// has a device for it, so there is no canonical spelling to translate to,
/// and a drawing set up to plot SVG here must come back set up the same way.
const SVG_DEVICE_NAME: &str = crate::ui::window::plot::OUT_SVG;
...
pub enum PlotDevice {
    ...
    Pdf,
    /// SVG file output (D5 of docs/plans/2026-09-07-dxf-to-svg-export.md);
    /// ours alone, see [`SVG_DEVICE_NAME`].
    Svg,
...
impl PlotDevice {
    ...
    pub fn canonical_name(&self) -> String {
        match self {
            ...
            PlotDevice::Svg => SVG_DEVICE_NAME.to_string(),
    ...
    pub fn from_stored_name(name: &str) -> (Self, bool) {
        ...
        if name == SVG_DEVICE_NAME {
            return (PlotDevice::Svg, true);
        }
    ...
    pub fn margins_mm(&self, paper: &PaperSize, custom: &[CustomPaper]) -> Margins {
        ...
        match self {
            ...
            // An SVG has no unprintable edge either: the whole sheet is the
            // drawing area, and `--margins` states any inset explicitly.
            PlotDevice::None | PlotDevice::Svg => user_defined.unwrap_or(Margins::ZERO),
        }
    }
}
