...
**文字。** 文字的字高、字体和颜色按这条文字自己的格式取——SmartPlant 里选中文字设的字号和字体（管道号、位号常见的是 7 磅 Arial Narrow），而不是它段落样式上的默认值（多为 1/8 英寸 Arial）；对齐与行距仍按段落样式。一条文字里几段格式不同时（管道号的号段与分隔符、`m^3` 里的上标 `3`），整条按覆盖字符最多的那段画，日志里记一行有几条这样压平。样式表给不出可用字高的文字（语料里是一种 0.254 mm 的占位样式）仍按 2.5 mm 画，日志里同样点名。
