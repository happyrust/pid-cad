...
**导入汇总。** 打开完成后命令行给出两行信息：第一行是画出了多少实体、来自多少条已解码记录、有多少条源记录没有画出，以及带原图图层的实体数；第二行是 `P&ID sheet layers: N, of which M start switched off`——按名字数的原图图层有几个、其中几个按文件里的隐藏状态初始关闭。图里有参数化符号时还有第三行 `P&ID driving dimensions: D on T template bodies; P placed parametric bodies carry library defaults`——文件缓存的符号库模板本体上有名字的驱动尺寸共几条、分布在几个模板本体上、有几个放置能在特性面板里看到这些库默认值（每个这样的放置在面板上对应一行「驱动尺寸（库默认）」）；没有参数化符号的图不出这一行。若样式表读取失败，还会多一行错误提示，此时线宽和颜色会退回图层默认。图纸没有声明坐标单位时也多一行（`P&ID import: the drawing states no coordinate unit; the metre was assumed.`）——坐标按米换算，英制图会整体差 25.4 倍，所以说出来；声明了单位的图不出这一行。这份摘要随文档一起交到打开完成的一步，读一次就取走，不会写进另存的 DWG / DXF；`--export` 这类没有命令行的路把它转进日志。逐条明细在日志里。

**符号从哪来。** 每个符号放置画的是这张 `.pid` 自己缓存的那份本体——SmartPlant 实际放置的那个符号版本，参数化符号则是被拉伸后的实例；符号内部处于关闭图层上的笔画（伴热线、夹套线、`NULL` 占位文字、参数化本体的构造线等，文件里标为不显示、SmartPlant 屏幕上也看不见）不画，也不进任何图层。符号内部哪几笔画成虚线，按符号自己存储里的样式表（如去外单元接续符号的虚线圆和四条虚线腿、呼吸阀的虚线唇），颜色与线宽仍按放置点名的样式。符号库只在文件没有可画的缓存本体时补位：`.pid` 通过工程参考数据共享路径引用符号库，程序会从图纸所在目录向上查找 SmartPlant 工程的 `Ref\Symbols`；找不到时，把环境变量 `PID_SYMBOL_LIBRARY` 指向本地的符号库副本。两边都没有本体的放置只显示一个位置点。日志里另有一行说明本次导入有多少放置画的是缓存本体、多少画的是库本体、跳过了多少条关闭层上的笔画；命中的符号库目录（`PID_SYMBOL_LIBRARY` 或找到的 `Ref\Symbols`）和读到的坐标单位也记在导入摘要里，同一张 `.pid` 换个目录画得不一样时由此可查。

**文字。** 文字的字高、字体和颜色按这条文字自己的格式取——SmartPlant 里选中文字设的字号和字体（管道号、位号常见的是 7 磅 Arial Narrow），而不是它段落样式上的默认值（多为 1/8 英寸 Arial）；对齐与行距仍按段落样式。一条文字里几段格式不同时（管道号的号段与分隔符、`m^3` 里的上标 `3`），整条按覆盖字符最多的那段画，日志里记一行有几条这样压平。样式表给不出可用字高的文字（语料里是一种 0.254 mm 的占位样式）仍按 2.5 mm 画，日志里同样点名。另存 DWG / DXF 时，这些文字样式写上字体文件名（如 Arial Narrow 写 `ARIALN.TTF`），重新打开或在别的 CAD 里看仍是同一种字体；本机没装的字体只能写 `txt`，导入日志里会点名。2026-09-24 之前另存的图里文字样式都是 `txt`，重新打开是笔画字体。

**图层。** 导入后每个实体的图层就是 SmartPlant 里它所在的图纸图层名——`Default` / `Labels` / `ConsistencyChecks` / `HiddenObjects` 等，不加前缀，同名图层跨存储合并为一层；图层表列出的是原图自己的全部图层（包括没有画出实体的），每层的开关按文件里的显示状态设定，原图隐藏的层直接为关，在图层管理器里可以像普通图层一样开关。程序自己造出的实体不在任何图纸图层上，留在各自的 `PID-*` 层上，这些层只在有实体落上去时才出现：

| 图层 | 内容 | 初始 |
|---|---|---|
| `PID-FRAME` | 按文件页面尺寸画出的图框 | 开 |
| `PID-SYMBOL-LABEL` | 符号名标签 | 关 |
| `PID-CONNECTIVITY` | 连通链诊断线 | 关 |
| `PID-GEOMETRY` | 没有原图图层的字形线（样式簇的字形笔画） | 开 |
| `PID-HIDDEN` | 原图隐藏、但文件没给出图层名的内容（语料四图里没有） | 关 |

分类信息没有丢：每个实体的 `PID_SEMANTICS` 扩展数据里写着 `role=`（角色：`geometry` / `text` / `symbol` / `symbol-label` / `point-ok` / `point-warning` / `point-error` / `point-approved` / `connectivity` / `fill` / `frame`）与 `style=`（原图样式名，如 `Primary Piping - New`），特性面板的「角色」行和图层管理器「图纸图层」视图下方的「角色」一段读的就是它。**另存为 DWG / DXF 后，第三方查看器里看到的图层名 = SmartPlant 原图图层名；要按导入器的分类取实体，读 XDATA `PID_SEMANTICS` 的 `role=` / `style=`。** 2026-09-21 之前另有一种「分类合成层」读法（`PID-GEOMETRY` / `PID-TEXT` / `PID-SYMBOL` / `PID-POINT-*` / `PID-STYLE-<样式名>` / `PID-HIDDEN` 各占一层，隐藏内容挪去 `PID-HIDDEN`）和在两种读法之间切换的环境变量，且以它为默认；两种读法画出来的图一模一样（颜色、线宽都在实体自己身上），只差图层表，已一并退役——设了那个变量也没有反应。

**图纸图层视图。** 打开 `.pid` 后，图层管理器的工具栏多出 `图层` / `图纸图层`（英文界面为 `Layers` / `Sheet layers`）两个切换按钮。切到 `图纸图层`，表格按原图图层（`Default` / `Labels` / `ConsistencyChecks` / `HiddenObjects` 等，不含程序自己的 `PID-*` 层）列出每层的实体数，下方 `角色`（Roles）一段列出各角色（`geometry` / `text` / `symbol` / `symbol-label` / `point-*` / `connectivity` / `fill` / `frame`）及实体数。点击行尾的眼睛可以单独关掉或打开某个原图图层或某个角色：一个实体只要所属图层或角色任一被关就不显示。这些开关记录在图纸里（`PID_VIEW_FILTER`），另存为 DWG/DXF 后再打开仍然有效，也可以撤销。原图隐藏的图层初始为关；把它打开时，程序会连带打开图层表里同名的那个图层（文件没给出名字的隐藏内容则是 `PID-HIDDEN`），实体才看得见；关掉不动图层表。搜索框对两种视图都有效。

**特性面板。** 选中导入的实体，左侧特性面板会多出 `P&ID` 一组只读属性：`类型`（已发布数据里的对象类，如 `PIDPipeline`）、`角色`（导入时读到的角色，即「图层」一段里 `role=` 的那套词）、`驱动尺寸（库默认）` 与 `本体尺寸`（仅符号放置的实体有，见下一段）、`位号` 或 `管线号`、`匹配方式`（若来自图例识别）、`图纸图层` 与 `图层 OID`（原图图层名及其在文件里的编号）。这些信息写在实体的 `PID_SEMANTICS` 扩展数据里，导出 DXF/DWG 时保留。

**符号的三个尺寸。** 点选一个符号放置的任一笔画或它旁边的符号名，`P&ID` 组会多出 `本体尺寸`——这张图上该符号本体的外框宽 × 高（毫米，两位小数，按放置的旋转 / 缩放量得，量的就是屏幕上画出的那些笔画——`.pid` 自己缓存的实例本体里文件显示着的那部分）；如果它是参数化符号且文件里配得上它的库模板，还会多出两行：`驱动尺寸（库默认）`——模板本体上按名字列出的驱动尺寸，如 `Top 20.32 mm · Left 114.30 mm · Right 114.30 mm`；`驱动尺寸（本图实例）`——这张图上该实例实际被放置成的参数，如同一个 Manifold 的 `Left 57.91 mm · Right 114.30 mm · Top 35.59 mm`（读自符号定义缓存里实例自己的 flavour 记录；没被拉伸过的实例这一行等于库默认）。**中间那行是符号库的默认值，不是这张图上的实际尺寸**：SmartPlant 放置后被拉伸过的实例，文件里那份变量副本仍等于库默认，实际参数另存一处，所以三行并排给出，分别是「库里怎么定义的」「这张图上放成了多大」「画出来的外框多大」。非参数化符号（阀门、仪表等）只有 `本体尺寸` 一行。三个值分别以 `extent=` / `driving=` / `instance=` 写在 `PID_SEMANTICS` 里，同一放置的所有实体相同，随 DWG/DXF 保留。
