...
**全图只有一个 marker**，也就是 23 个放置里只有 1 个没解出本体——不存在"六个符号退化成什么都没有"这回事。
...
值得记一笔：pid-parse 自己的模型说的是另一回事——`symbol_library.rs` 的
`valve_symbol_reads_as_lines_and_circles_around_its_own_origin` 断言本体在原点 ±20mm 内，
`COORDINATE_LIMIT` 的注释也写着"参考库自身的 extent 在 0.6 以下"（0.6m ≈ 一张图幅）。
上表这六个符号把那条断言破了 5~8 倍。
...
倾向甲：按乙的话，一张正式图纸会把六个符号画到纸外面去，不合常理。但这正是本项目栽过三次的
那种推理（A01 那 18 条、GLine2d 锚点、JStyleOverride 锚点，都是"看着规整"先下结论后被推翻），
所以不作结论。而且乙有一条硬支撑：pid-parse 自己的断言就是"本体绕自身原点"，这六个符号破了它，
说明 `.sym` 里**确实有没读懂的东西**。
