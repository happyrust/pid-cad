...
> 七条决策按推荐执行。H4 的 `--export` 对数已做、四图对上事实表；GUI 截图未截（桌面被占，见进度），留待补——**2026-09-24 已补**：图层管理器「图层」视图列的是原图图层名、`HiddenObjects` 等按文件显示位为关，
> 见 `docs/evidence/2026-09-24-pid-gui-check/0201-layer-manager.png`（计划 `2026-09-24-pid-import-status-and-next-steps.md` V1）。
