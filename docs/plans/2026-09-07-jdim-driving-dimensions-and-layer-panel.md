...
> `pid_import` 44 → 45，pid-parse `--lib` 1094 → 1097、`parse_real_files` 127 → 128。
>
> **同夜续**：**J1 四步走完**（pid-parse `a8b8dd1` → `1b30dff` → `477fb72` → `1319f10`）——`0x0115` 的帧 18/18 收尾
...
**进度**：**① ✅** 2026-09-14 pid-parse `a8b8dd1`（`probe(jdim): lay the 18 JDim records out field by field, and close the length account`）——
`examples/probe_jdim_bytes.rs` 把 18 条（D06 5 / 0201 5 / 工艺 4 / A01 4；0202 依旧 0 条）逐条摆开：信封、头部字、
尺寸值、块区引用槽、f64 扫描、带批注的十六进制。记录层面定下两件：
**长度账 18/18 精确收尾**——`payload = 34 + main_len(+30) + tail`，`tail` 为 4 字节**当且仅当** `+26` 标志字含 `0x0300`；
194 与 198 之差就是这一个尾字（主块同为 160），166 / 276 / 288 / 308 同一条规则一并收尾。标志字比 09-07 多出一个值
`0x0041`（A01）：无尾的是 `0x0041` / `0x0051`，带尾的是 `0x0341` / `0x0351` / `0x0361`。
**引用槽落到具名对象**——`+82` 是块数（短三形 1、长三形 2），`+92` 在 **18/18** 里指向 `Line Object`（`0x0018`）或
`Point Object`（`0x005E`），就是这条尺寸量的几何；其后的标记字说的正是类（`0x00CB` ↔ 线 20/20、`0x00F0` ↔ 点 4/4）。
`+140` 在 14/18 里指向约束对象（`0x0085 Vertical Constraint` / `0x0089` / `0x0114` / `0x0058`；它的标记字 `0x0067` / `0x008C`
**不**跟类走）。`+202` 恰好出现在那 6 条双块记录里。08-27 tag-188 那 42 条命中由此有了落点。
**留给第 ② 步的**：块内文法还没有一条读法同时盖住 102（198 形）/ 70（166 形）/ 82（288 的第二块）三种块长；
`+34` / `+50` / `+58` 三个 f64 恒在 2^-15 与 2^-11 米量级、读起来像公差；`+18` 其实是 u16 版本号 2（其后一个 u16 为 0 或 1，
...
**③ ✅ 2026-09-14 pid-parse `477fb72`**（`tools/idalib_radsrv_igdimension.py` + 探针改口径）——**原生读取器认账了**。
`radsrvitem.dll` 的 igDimension 链（`sub_564BA320` → 五个子读取器，08-04 只走到「有这么一条链」）读的正是同一批字节，
于是第 ① 步那本长度账**从「语料拟合」变成「读取器自己的算术」**：
- **基址对上**：`sub_564BB990` 头一句 `*(WORD*)a2 == 277`，所以 `a2` 是**记录头**，它的每个偏移都比 payload 小 6。
- 于是 `a2+32` = payload **`+26`**（标志字），而尾字它取自 `*(u32*)(a2 + 40 + *(u32*)(a2 + 36))`
  = payload **`+34+main_len`**，正是记录最后四字节——**且只在 `0x100` 位下取**。探针原先按 `0x0300` 判是因为
  语料里这两位从不单独出现；改成 `0x100` 后**仍 18/18 收尾**。
- **`+34` 是块首**，`sub_56446B50` 给块长：kind 1/8 是 **48**（块首 dword 含 `0x2000` 时 80），其余 48…64，kind 7 是 52。
  48 正是「为什么 `+82` 是下一段的起点」。
- **`+14` 那个 u16 不是 index，是尺寸种类（kind）**：第二个子读取器按它在 **8 个值**之间分派，每种一个块读取器。
  语料 18 条全是 kind 1——**八种里只练到一种**，这解释了第 ① 步「一条读法盖不住三种块长」：块长本来就随 kind 走。
**仍未坐实**：第一个子读取器从同一 dword 里展开约十六个位标志（`0x40`、符号位、`0x200`…`0x100000`）并把 `+14`
过一张重映射表（`sub_564B82A0`），语料里变化不够、分不开。**imagdex 的 DoIO 仍未找到**（见第 ② 步），
但 J1 验收的「六种长度按同一读法精确收尾」已由原生读法背书。

**④ ✅ 2026-09-14 pid-parse `1319f10`**——分析文档
`docs/analysis/2026-09-14-jdim-is-a-framed-record-whose-blocks-follow-the-dimension-kind.md`
（§7 就是计划要的「坐实 / 留 raw」两栏），连 CHANGELOG、`task_plan.md` 指针、guide 的 `0x0115` 行
（从「语料 0 命中」改成「18 条、坐在文件说关的 `Dimension` 层上、帧已解、无解码器仍丢弃」）。
**坐实栏**：帧 `34 + main_len + 尾字(0x0100)`、`+30 main_len`、`+14` 尺寸种类（8 种）、`+34` 块首与按种类的块长、
`+42` 尺寸值（米）、`+92` 被量的几何（标记字标类）、`+140` 归属的组 / 约束 / 属性集。
**留 raw 栏**：`+22`、标志字除 `0x0100` 外约十六个位、`+20` 的重映射码、三个疑似公差的 f64、
`+82` 之后每段的完整文法（三对点 / 轴向 / `π/2` / 段长 102·70·82 之别）、`0x00CB` / `0x00F0` 两个标记字的类名。
**按 D3 纪律，留 raw 栏不进 DTO，所以 J1 落地时不写解码器。**

**J1 结算（对照验收三条）**：
① **「六种长度全部按同一读法精确收尾」达成**，且不是语料拟合——原生读取器自己的算术。
② **「尺寸值与三对点的语义有原生读法背书」只做到一半**：`+42` 是尺寸值只有语料证据（18/18 英寸整倍，
且与 09-07 那条 `Parametric Manifold` 35.59 mm 对得上），三对点**没有**原生背书——原生链给的是帧与按种类分派，
不是块内语义；imagdex 的 DoIO 是块内语义的唯一权威来源，而它没找到（第 ② 步）。
③ **「tag-188 的 42 条命中落到具名字段」有了落点但未逐条核对**：`+92` / `+140` / `+202` 三个引用槽已具名，
把 42 条逐条对到槽上是 J2 的活。
**时间盒**：两个工作日的盒子只用了一天多；剩下的缺口不是再多花时间就能补的（缺的是 imagdex 那个读取器），
按 D3「没有原生读法坐实的字段不进 DTO」收在这里比硬凑读法好。
