...
> 结论：P6.2–P6.4 不做）、**P4.2 已实施**（§10）、**P4.3 已实施**（§11）、**P7 已实施**（§12）、**P5.2 已实施**（§13，真图证据跑仍欠）；剩 P5.3 与 G10。
...
- 证据落盘：**半** ✓ ——语料那张齐（`corpus-600dpi.tsv` 22 行、非零对的 diff 图、最小对照、README）；
  **真图那张 `real-sheets-600dpi.tsv` 还没跑**：`dump_real_sheet_raster_evidence` 就位未运行（600 dpi × 三张，
  A1 是 GB 级位图 ×2 引擎，要单线程手动跑：`cargo test --lib dump_real_sheet_raster_evidence -- --ignored
  --nocapture --test-threads=1`）。补跑之后把数字记回本节，README 里那一节同步。
...
600 dpi 证据表 22 行如上；**真图三张的证据跑还没做**（见出口对照第四条，300 dpi 的试跑只留在
`automation.rs` 的注释里，没有表）。rustfmt：`raster_compare.rs` / `tests.rs` 整文件 0 diff
