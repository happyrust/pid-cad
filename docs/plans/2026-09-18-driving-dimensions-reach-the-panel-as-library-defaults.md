...
> `extent=` 量缓存实例本体、`driving=` 限配上模板的放置，`pid_import` 51/51 × 两模式（见 K2 进度与结算）。**K3 落地**：OCS `06c95ce1`，摘要第三行全零不推，0201 (4, 2, 2)（见 K3 进度与结算）。**K4 收口**：双仓台账齐，本计划关闭；开口见「结算」。
...
**进度（2026-09-19，fable-5-1-17）**：随项落地的部分——user-guide 面板句与「符号的两个尺寸」段（`39398579`）、导入汇总第三行
（`06c95ce1`）；pid-parse CHANGELOG / guide / task_plan（`5d64d7c`）；本计划各项进度与结算（`4d51cef3` / `436b4baa` / `6e8dcbe6`）。
本次收口：09-07 计划 D7 结算行标 K1 → K4 全落、D2 后半句结清；pid-parse `task_plan.md` 指针改「本计划收口」；`remember`
`mem-221`（K1 配对判据 + CRLF 踩坑，K1 时已记）与 `mem-231`（键名 / 库默认口径 / 显示优先级开口 / OCS fmt 踩坑）。

---

## 结算（2026-09-19）

**四项齐。** 目标那句话兑现到什么程度：选中 0201 的 Manifold 任一笔画或它的名字，`P&ID` 组给「驱动尺寸（库默认）
`Top 20.32 mm · Left 114.30 mm · Right 114.30 mm`」与「本体尺寸 `172.21 × 71.18 mm`」两行——一个是库里怎么定义的、一个是这张图上
画了多大，两个数分开、都不撒谎；点一个阀门只有「本体尺寸」；打开时命令行第三行 `P&ID driving dimensions: 4 on 2 template bodies;
2 placed parametric bodies carry library defaults`，0202 没这一行。

| 项 | 提交 | 验收 |
|---|---|---|
| K1 | pid-parse `223b26d` + `5d64d7c` | 5/5 配上（4 by `value_ref`、1 by 名 + 值）、15/18 有名、无名恰三条；`--lib` 1110、`parse_real_files` 130 → 131、golden 不变、clippy `-D warnings` 零告警 |
| K2 | OCS `39398579` + `436b4baa` | 四个参数化放置钉值、每个符号实体有 `extent=`、非参数化无 `driving=`、两键过 DWG/DXF、两模式五键一致；21 本词条；`pid_import` 51/51 × 2 |
| K3 | OCS `06c95ce1` + `6e8dcbe6` | 0201 (4, 2, 2) / D06 (4, 1, 1) / 工艺 (4, 1, 2) / 0202 (0, 0, 0)，第三数 = 带 `driving=` 的符号名标签数 |
| K4 | 本提交 + 上列文档提交 | 双仓台账齐 |

**与计划字面不同的两处**（都已写进各项进度）：K1 第三步配对改按模板本体的线 / 弧数在实例存储里挑（记录写序在工艺那对上不成立）；
K2 `extent=` 量缓存实例本体而不是 `built`（库在场时 `built` 是模板形）。
...
- 2026-09-19：K4 收口（本提交，同一会话）——09-07 计划 D7 结算行、pid-parse task_plan 指针、`remember`；四项齐，计划关闭。
